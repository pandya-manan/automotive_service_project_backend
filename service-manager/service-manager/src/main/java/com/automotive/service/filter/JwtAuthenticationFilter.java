package com.automotive.service.filter;
...
import com.automotive.service.entity.Role;
import com.automotive.service.util.JwtTokenUtil;
...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        try {
            ...
            // Check role - only SERVICE_MANAGER role allowed
            Role userRole = jwtTokenUtil.getRoleFromToken(token);
            if (userRole != Role.SERVICE_MANAGER) {
                response.setStatus(HttpServletResponse.SC_FORBIDDEN);
                response.getWriter().write("{\"error\":\"Access denied. SERVICE_MANAGER role required.\"}");
                response.setContentType("application/json");
                return;
            }
